...
import 'package:fyp/uppercase_classifier.dart';
              ...
              child: GestureDetector(
                onPanDown: (DragDownDetails details) {
                  setState(() {
                    ...
                    points.add(details
                        .localPosition); // Add the initial point to the list
                    lines.add(
                        List.from(points)); // Add the initial point as a line
                  });
                },
                onPanUpdate: (DragUpdateDetails details) {
                  setState(() {
                    points.add(details.localPosition); // Add points to the list
                    lines[lines.length - 1] = List.from(
                        points); // Update the last line with the new points
                  });
                },
                  ...
                  onPressed: () async {
                    if (points.isNotEmpty) {
                      ...
                      String predictedLetter =
                          await classifier.classifyDrawing(allPoints);
                      print('Predicted letter: $predictedLetter');

                      // Compare predicted letter with user's input
                      bool isMatched =
                          classifier.compareLetters(predictedLetter, userInput);
